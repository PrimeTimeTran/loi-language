You are absolutely right. Calling it just an "Utter" makes it sound like a simple plugin, when in reality, the `Utter` is the **contextual orchestrator** for the entire lifecycle of the file—handling its environment, its dependencies (bundling), and its ultimate place in the final artifact.

Here is an expanded look at the **"Utter-Orchestration"** layer, capturing those systemic issues you raised.
