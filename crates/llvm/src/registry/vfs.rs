...
impl InodeOperations for InMemoryFileInode {
    ...
    fn get_attr(&self) -> Result<FileAttr, VfsError> {
        Ok(self.attr.read().unwrap().clone())
    }
}
...
impl FileOperations for InMemoryFileInode {
    ...
    fn write(&self, buf: &[u8], offset: u64) -> Result<usize, VfsError> {
        ...
        self.attr.write().unwrap().size = data.len() as u64;
...
#[wasm_bindgen]
pub fn init_vfs_from_json(root_data: JsValue) -> Result<VfsHandle, JsValue> {
    let json: JsonNode = serde_wasm_bindgen::from_value(root_data).map_err(|e| e.to_string())?;

    fn build_node(node: JsonNode) -> Arc<Dentry> {
...
impl Vfs {
    fn resolve(&self, path: &str) -> Result<Arc<Dentry>, VfsError> {
...
impl Vfs {
    fn parent(&self, path: &str) -> Result<(Arc<Dentry>, String), VfsError> {
...
fn build_vfs(root: JsonNode) -> Arc<Dentry> {
    fn build_node(node: JsonNode) -> Arc<Dentry> {
        let inode: Arc<dyn Inode> = if node.r#type == "directory" {
            Arc::new(InMemoryDirectoryInode::new())
        } else {
...
impl Vfs {
    ...
    pub fn new(data: JsValue) -> Result<Vfs, JsValue> {
        let json: JsonNode = serde_wasm_bindgen::from_value(data).map_err(|e| e.to_string())?;
...
impl Vfs {
    pub fn stat(&self, path: String) -> Result<FileStat, JsValue> {
        ...
        Ok(FileStat {
            ...
            is_dir: node.inode.file_ops().read(&mut [], 0).is_err(),
        })
    }
